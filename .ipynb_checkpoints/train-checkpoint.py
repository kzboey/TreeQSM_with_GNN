...
def train(model, loader, model_name):
    optimizer = torch.optim.Adam(model.parameters(),
                              lr=lr,
                              weight_decay=decay)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=step_size, gamma=scheduler_decay)
    current_patience = 0
...
def report(model, loader, model_name):
    ...
    for i, data in enumerate(tqdm(loader)):
        x, edge_index, batch, targets = data.x.to(device), data.edge_index.to(device), data.batch.to(device), data.y.to(device)
        _, out = model(x, edge_index, batch)
        y_label = y_label + targets.to('cpu').numpy().flatten().tolist()
        y_pred = y_pred + out.argmax(dim=1).to('cpu').numpy().flatten().tolist()

    print(classification_report(y_label, y_pred, target_names=config.target_names))
...
train_dataset = files[:int(len(files)*0.7)]
val_dataset   = files[int(len(files)*0.7):int(len(files)*0.9)]
test_dataset  = files[int(len(files)*0.9):]
...
classification_loader = DataLoader(test_dataset, batch_size=config.batchsize, shuffle=False)
...
for (model, model_name) in models:
    ...
    test_loss, test_acc, test_f1 = test(best_model, test_loader, model_name)
    print(f'Test Loss: {test_loss:.2f} | Test Acc: {test_acc*100:.2f}% | Test F1 Acc: {test_f1*100:.2f}%')
    report(best_model, classification_loader, model_name)
